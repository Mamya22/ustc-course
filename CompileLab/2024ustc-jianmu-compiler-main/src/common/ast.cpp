  ...
  */
  if (_STR_EQ(n->name, "program")) {
    ...
    */
    ...
    std::stack<syntax_tree_node *>
        s;
    // n->children[3]->name 一定是 params
    auto list_ptr = n->children[3];
    
    //如果params为空
    if(_STR_EQ(list_ptr->children[0]->name, "void")){
      // node->params.push_back(nullptr);
    }
    else{ //list_ptr->childen[0] = param_list
      while(list_ptr->children[0]->children_num == 3){
        s.push(list_ptr->children[0]->children[2]);
        list_ptr = list_ptr->children[0];
      }
      s.push(list_ptr->children[0]->children[0]);
      while(!s.empty()){
    ...
    */
    ...
    std::stack<syntax_tree_node *> stack_1;
    auto ptr_local = n->children[1];
    while(ptr_local->children_num == 2){
      stack_1.push(ptr_local->children[1]);
      ptr_local = ptr_local->children[0];
    }
    while(!stack_1.empty()){
      auto child_node = static_cast<ASTVarDeclaration*>(transform_node_iter(stack_1.top()));
    ...
    auto ptr_sta = n->children[2];
    std::stack<syntax_tree_node*> stack_2;
    while(ptr_sta->children_num==2){
      stack_2.push(ptr_sta->children[1]);
      ptr_sta = ptr_sta->children[0];
    }
    while(!stack_2.empty()){
      auto child_node_2= static_cast<ASTStatement*>(transform_node_iter(stack_2.top()));
      auto child_node_shared_2 = std::shared_ptr<ASTStatement>(child_node_2);
      node->statement_list.push_back(child_node_shared_2);
      stack_2.pop();
    }
    ...
    */
    // 5.1 expresstion
    node->expression = std::shared_ptr<ASTExpression>(static_cast<ASTExpression*>(transform_node_iter(n->children[2])));

    // 5.2 if statement
    node->if_statement = std::shared_ptr<ASTStatement>(static_cast<ASTStatement*>(transform_node_iter(n->children[4])));

    // check whether this selection statement contains
    // 5.3 else structure
    if(n->children_num == 7){
      node->else_statement = std::shared_ptr<ASTStatement>(static_cast<ASTStatement*>(transform_node_iter(n->children[6])));
    }
    else{
    ...
    if (n->children_num == 3) {
      ...
      */
      ...
      auto additive_node=static_cast<ASTAdditiveExpression*>(transform_node_iter(n->children[0]));
      node->additive_expression = std::shared_ptr<ASTAdditiveExpression>(additive_node);
    ...
    if (_STR_EQ(name, "expression") || _STR_EQ(name, "var") ||
      ...
      */
      if (_STR_EQ(name, "integer")) {
        ...
        int num = 0;
        int j = 0;
        syntax_tree_node *temp_node = n->children[0]->children[0];

        while(temp_node->name[j]!=0 && j < 30){
          num = num * 10 + temp_node->name[j] - '0';
          j++;
        }
        num_node->i_val = num;
        num_node->type = TYPE_INT;
        ...
        syntax_tree_node *temp_node = n->children[0]->children[0];
        int j = 0;
        while(temp_node->name[j]!='.' && j < 30){
          int_num = int_num * 10 + temp_node->name[j] - '0';
          j++;
        }
        j++;
        double len = 1;
        while(j < 30 && temp_node->name[j]!=0){
          float_num = float_num*10 + temp_node->name[j] - '0';
          len = len * 0.1;
          j++;
        }
        num_node->f_val = int_num + float_num * len;
