...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("lu12i.w $t0, 0x3f800");
    codegen->append_inst("movgr2fr.w $ft1, $t0");
    // TODO: 使用 fcmp.slt.s 进行比较, 比较结果在浮点标志寄存器中, 你需要思考
    // 如何将浮点标志寄存器中的值写入内存. 提示: 尝试使用 bcnez 指令
    codegen->append_inst("fcmp.sle.s $fcc0, $ft1, $ft0");
    codegen->append_inst("bcnez $fcc0, true_label");
    // 存储0
    codegen->append_inst("st.b", {"$zero", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("b extent");
    // 存储1
    ...
    codegen->append_inst("extent", ASMInstruction::Label);
    ...
    // codegen->append_inst("label3", ASMInstruction::Label);
    /* label3: */
